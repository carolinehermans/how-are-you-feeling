...
for word in oldHuntWords:
    huntWords.append(word.lower())

huntDict = {}
for word in huntWords:
    if word in huntDict:
        huntDict[word]+=1
    else:
        huntDict[word]=1
...
topTenHunt = []
inTopTen = False
noWords = ["to", "and", "i", "for", "the", "my", "a", "is", "of", "but", "i'm", "in", "it", "it's", "am"]
for word in huntDict:
    if len(topTenHunt)<10 and word not in noWords:
        if notInTop(word, topTenHunt): topTenHunt.append((word,huntDict[word]))
    for winner in topTenHunt:
        if winner[1] < huntDict[word] and len(topTenHunt)>=10 and word not in noWords:
            if notInTop(word, topTenHunt):
                topTenHunt.remove(winner)
                topTenHunt.append((word,huntDict[word]))
...
oldfbWords = fb.split()
fbWords = []
for word in oldfbWords:
    fbWords.append(word.lower())
...
topTenfb = []
inTopTen = False
noWords = ["to",  "so", "and", "i", "for", "the", "my", "a", "is", "of", "but", "i'm", "in", "it", "it's", "am"]
for word in fbDict:
    if len(topTenfb)<10 and word not in noWords:
        if notInTop(word, topTenfb): topTenfb.append((word,fbDict[word]))
    for winner in topTenfb:
        if winner[1] < fbDict[word] and len(topTenfb)>=10 and word not in noWords:
            if notInTop(word, topTenfb):
                topTenfb.remove(winner)
                topTenfb.append((word,fbDict[word]))

print "\n***** Facebook Most Frequent Words *****"
for elem in topTenfb:
    print elem[0] + ", " + str(elem[1])

tot = hunt + " " + fb

oldtotWords = tot.split()
totWords = []
for word in oldtotWords:
    totWords.append(word.lower())
...
noWords = ["to",  "so", "and", "i", "for", "the", "my", "a", "is", "of", "but", "i'm", "in", "it", "it's", "am"]
for word in totDict:
    if len(topTentot)<10 and word not in noWords:
        if notInTop(word, topTentot): topTentot.append((word,totDict[word]))
    for winner in topTentot:
        if winner[1] < totDict[word] and len(topTentot)>=10 and word not in noWords:
            if notInTop(word, topTentot):
                topTentot.remove(winner)
                topTentot.append((word,totDict[word]))

print "\n***** All Responses Most Frequent Words *****"
for elem in topTentot:
    print elem[0] + ", " + str(elem[1])
